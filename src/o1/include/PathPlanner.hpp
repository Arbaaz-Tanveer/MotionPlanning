...
const double ROBOT_RADIUS = 0.3;    // Example: 30cm radius for your robot
const double OBSTACLE_RADIUS = 0.3; // Example: 30cm radius for other robots
    ...
    double clearance(const ob::State *state) const override
{
    ...
    // Calculate the minimum distance required to be "clear"
    const double total_collision_radius = ROBOT_RADIUS + OBSTACLE_RADIUS;

    // Check clearance against all obstacles
    for (const auto &obs_point : obstacles)
    {
        // Calculate the distance from the state (x, y) to the obstacle's center
        double distance_to_obstacle = sqrt(pow(x - obs_point.x, 2) + pow(y - obs_point.y, 2));

        // If the distance is less than the combined radii, it's a collision.
        // The state is invalid, so return a negative clearance.
        if (distance_to_obstacle < total_collision_radius)
        {
            return -1.0;
        }
    }

    // If we checked all obstacles and there were no collisions, the state is valid.
    return 1.0;
